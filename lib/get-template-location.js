...
function getTemplateLocation(moduleName, { currentAddonPath, globConfig, isApp = true } = {}) {
  if (isApp) {
    ...
    const STATIC_DIRS = ["core", "extended", "engine-lib", "lib"];
    let globPatterns = STATIC_DIRS.map(pattern => pattern + globPattern);
    // console.log(globPatterns);
    let templateLocations = globby.sync(globPatterns, globConfig);
    ...
    if (templateLocations.length <= 0) {
      ...
      globPatterns = STATIC_DIRS.map(pattern => pattern + globPattern);
